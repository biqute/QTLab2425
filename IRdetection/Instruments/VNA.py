...
class VNA:
    def __init__(self, resource_name='GPIB0::16::INSTR'):
        ...
        self.set_array_output_format('float32')
    ...
    def get_sweep_data(self):
        ...
        """
        raw_data = self.query_raw('OUTPDATA')
    ...
    def set_array_output_format(self, format='float64'):
        """
    ...
    def decode_raw(self, raw_data):
        ...
        # Read ACSII
        if self.array_format == 4:
            data = np.array([float(x) for x in raw_data.decode('utf-8').split(',')])
            real = data[::2]
            imag = data[1::2]
            data = real + 1j * imag
            return data
...
vna.set_array_output_format('float32')
#print(vna.get_array_format())
print(vna.get_sweep_data())