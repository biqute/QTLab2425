...
def plot_fit(data, model, title: str = 'Model Fit', palette: Palette = None, typography: Typography = None, 
             fig=None, X_label: str = 'X-axis', Y_label: str = 'Y-axis', show_residuals: bool = True, 
    ...
    show_residuals : bool, optional
        Whether to show residuals plot below main plot. Default is True.
    ...
    **kwargs
    ...
    if fig is None:
        ...
        # Use existing figure - assume it has the right subplot structure
        axes = fig.get_axes()
        ax_main = axes[0]
        ax_residuals = axes[1] if len(axes) > 1 and show_residuals else None
    ...
    ax_main.scatter(x_data, y_data, color=scatter_color, alpha=0.7, s=50, 
                   label='Data', zorder=3, edgecolors='white', linewidth=0.5)
    ...
    ax_main.plot(x_fit, y_fit, color=line_color, linewidth=2.5, 
                label='Model', zorder=2, alpha=0.9)
    
    # Calculate residuals
    y_model_data = model(x_data)
    ...
    if not show_residuals:
        ax_main.set_xlabel(X_label, **xlabel_kwargs)
    ...
    if show_residuals and ax_residuals is not None:
        ...
        ax_residuals.scatter(x_data, residuals, color=scatter_color, alpha=0.7, s=30,
        ...
        ax_residuals.grid(True, alpha=0.5, linestyle='--')
