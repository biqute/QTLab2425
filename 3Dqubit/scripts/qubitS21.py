...
T = 300 # 64.6e-3 # read_temperature() # K

# N9916A
myVNA = VNA("192.168.40.10")
myVNA.min_freq = 7.29e9 - 500e6 # 7.4297463e9 - 50e6 # hot
myVNA.max_freq = 7.29e9 + 500e6 # 7.4297463e9 + 50e6 # hot
...
for Sij in ["S21"]: #["S11", "S12", "S21", "S22"]:
    # basename = f"../data/qubit_hot_3PC/run_fixed_power/{int(T * 1e3)}mK_{myVNA.bandwidth / 1e3}kHz_{myVNA.power}dBm_span_{myVNA.max_freq - myVNA.min_freq}_S21"
    basename = f"../data/qubit_hot_3PC/run_fixed_power/{int(T * 1e3)}mK_{myVNA.bandwidth / 1e3}kHz_{myVNA.power}dBm_span_{myVNA.max_freq - myVNA.min_freq}_S11"
    # basename = f"../data/qubit_hot_3PC/{int(round(T * 1e3))}mK_{myVNA.bandwidth / 1e3}kHz_{myVNA.power}dBm_S21"
    # basename = f"../data/qubit_hot_3PC/{int(round(T * 1e3))}mK_{myVNA.bandwidth / 1e3}kHz_{myVNA.power}dBm_S21_wide"
    ...
    # Metadata
    metadata = {
        "info": "cavity with qubit, three port configuration for S11 = S_1 + H_o", 
        #"info": f"cavity with qubit, three port configuration for S21 = H_i + H_o", 
